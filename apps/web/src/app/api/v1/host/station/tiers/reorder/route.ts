...
import {
  ...
  reorderStationPriorityTiers,
} from "@/lib/server-state";
import { Role } from "@platform/types";
import { reorderStationPriorityTiersSchema } from "@platform/validation";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = reorderStationPriorityTiersSchema.safeParse(rawBody);
    ...
    const updatedTiers = reorderStationPriorityTiers(
      station.id,
      validationResult.data.tierIds,
    );

    return NextResponse.json(updatedTiers);
  } catch (error: any) {
    return NextResponse.json(
      { message: error?.message || "Failed to reorder priority tiers", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}

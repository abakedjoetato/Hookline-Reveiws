...
describe("Host Manual Tier Change", () => {
  ...
  it("should change tier from Free to Priority and preserve Free origin", async () => {
    ...
    mockPrisma.livePriorityTierSnapshot.findFirst.mockResolvedValue({ id: "ts_1", priorityRank: 10 });

    await service.changeEntryTier("host_1", "ls_1", "qe_1", { destinationType: "PRIORITY_TIER", tierSnapshotId: "ts_1" });

    expect(mockPrisma.queueEntry.update).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("should restore Free origin when changing back from Priority to Free", async () => {
    ...
    await service.changeEntryTier("host_1", "ls_1", "qe_1", { destinationType: "FREE" });

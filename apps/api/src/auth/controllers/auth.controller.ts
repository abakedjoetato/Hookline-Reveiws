...
export class AuthController {
  ...
  async login(@Body() input: LoginInput, @Req() req: Request, @Res({ passthrough: true }) res: Response) {
    ...
    const { cookie } = await this.authService.login(input, ipAddress, userAgent);
  ...
  async logout(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    ...
    const cookieSvc = new CookieService(new (await import("@nestjs/config")).ConfigService());
    const rawToken = cookieSvc.parseSessionToken(cookieHeader);

    if (rawToken) {
        const clearCookie = await this.sessionService.revokeSession(rawToken, ipAddress);
  ...
  async logoutAll(@CurrentUser() user: AuthenticatedUser, @Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const ipAddress = req.ip || req.socket.remoteAddress || "unknown";
    const clearCookie = await this.sessionService.revokeAllSessions(user.id, ipAddress);
    res.setHeader("Set-Cookie", clearCookie);
    return { success: true };
  }
  ...
  async verifyEmail(@Body() input: EmailVerificationConfirmInput, @Req() req: Request) {
    const ipAddress = req.ip || req.socket.remoteAddress || "unknown";
  ...
  async requestPasswordReset(@Body() input: PasswordResetRequestInput, @Req() req: Request) {
    const ipAddress = req.ip || req.socket.remoteAddress || "unknown";
  ...
  async confirmPasswordReset(@Body() input: PasswordResetConfirmInput, @Req() req: Request) {
    const ipAddress = req.ip || req.socket.remoteAddress || "unknown";

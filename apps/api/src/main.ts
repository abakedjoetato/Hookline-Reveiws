...
async function bootstrap() {
  ...
  try {
    ...
    logger.error('❌ Failed to start API: Environment validation failed', error);
    process.exit(1);
  }
  ...
  app.use((req: any, res: any, next: () => void) => {
    const requestId = req.headers['x-request-id'] || req.headers['x-correlation-id'] || Math.random().toString(36).substring(2, 15);
  ...
  if (enableSwagger) {
    const config = new DocumentBuilder()
      ...
      .setDescription('Production-grade Backend API documentation for TheQueue platform')
      .setVersion('1.0')
      .addBearerAuth()
      .build();
    const document = SwaggerModule.createDocument(app, config);
    SwaggerModule.setup('docs', app, document);
    logger.info(`Swagger documentation initialized at http://localhost:${APP_PORTS.api}/docs`);
  }
